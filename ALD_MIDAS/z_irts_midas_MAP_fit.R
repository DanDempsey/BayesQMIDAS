...
irts_midas_MAP.fit <- function(x) {
  ...
  ### Do nothing if user requested a dry run
    ...
    x$opt <- list(message = "Dry run: no optimisation performed", par = par)
  }
  ...
  x$glm_diagnostics <- dry_glm_fit(WX, environment(form)$yv, start = split(x$par, x$pinds)[[1]], family = x$family)
