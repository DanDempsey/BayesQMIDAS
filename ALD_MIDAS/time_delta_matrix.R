...
if (FALSE) {
time_delta_matrix <- function(regime_object, response_index = regime_object$response_index) {
  
  # Calculate differences between the response indices and the covariate indices
  if ( missing(response_index) )
    response_index <- regime_object$response_index
  xt <- regime_object$series$time
  rownum <- length(response_index)
  colnum <- length(xt)
  yt <- matrix( rep(response_index, each = colnum), nrow = rownum, byrow = TRUE )
  diff_mat <- sweep(yt, 2, xt)
  
  # Determine which values are inside the time window, store results inside a sparse matrix
  within <- ( 0 <= diff_mat ) & ( diff_mat < regime_object$time_window )
  inds <- which(within, arr.ind = TRUE)
  regime_object$time_window_list <- list(nrow = rownum, ncol = colnum, i = inds[, 1], j = inds[, 2], 
