...
MIDAS_start <- function(formula, Zenv) {
  ...
  if ( is.null(get("start", Zenv)) )
    start <- glm.fit(startm, Zenv$yv, family = get("family", Zenv))$coefficients
...
MIDAS_start_ald <- function(formula, start, prior, Zenv) {
  ...
  if ( length(start) < length(pinds) ) {
    ...
    start <- list( beta = prior$beta, z = ((-1)^(1 - Zenv$yv))*rep(0.01, n_y), nu = rep(1, n_y) )
  }
...
MIDAS_start_ald_vs <- function(formula, start, prior, Zenv) {
