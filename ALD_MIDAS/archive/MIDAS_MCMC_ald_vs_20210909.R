...
all_DLF_prior <- function(x, m = c(0, 0), s = c(0.5, 0.5)) {
  sum( dnorm(x[1], mean = m[1], sd = s[1], log = TRUE), 
       log(dtruncnorm(x[2], b = 0, mean = m[2], sd = s[2])) )
}

##### Main function that performs inference via MCMC
MIDAS_MCMC_ald_vs <- function(formula, start, prior, 
                              varsel_prior, quantile, 
                              adapt, n_cores, MCMC_length, 
                              nburn) {
  ...
  var_inds <- 1:nvar
  accept <- accept_start <- runs <- runs_start <- rep( 1, nvar )
  ...
  names(sigmaDLF) <- names(runs) <- names(accept) <-  names(birth_oppurtunity) <- 
    ...
    names(runs_start) <- names(sigmaDLF_start) <- names(accept_start) <- 
    varnames 
  ...
  for ( i in 2:MCMC_length ) {
    ...
    for (j in which(varsel)) {
      ...
      l_r_denom <- ll + all_DLF_prior( DLF_last )
      
      # Proposal
      DLF_trans_last[2] <- log( -DLF_trans_last[2] )
      DLF_star <- rnorm( 2, DLF_trans_last, sigmaDLF[j] )
      ...
      l_r_num <- ll_star + all_DLF_prior( DLF_star ) + ljacob
  ...
  list(betares = betares, DLFres = DLFres, vsres = vsres,
       ...
       accept_start = accept_start, runs_start = runs_start, 
