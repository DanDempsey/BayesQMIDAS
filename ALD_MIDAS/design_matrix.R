...
midas_design_matrix <- function(regime_object) {
  
  # Compute the weight matrix
  weight_mat_list <- Map( weight_matrix, time_window_list = regime_object$time_delta,
                          MoreArgs = list(DLF = regime_object$DLF, DLF_parameters = regime_object$DLF_parameters) )
...
midas_design_matrices <- function(regime_object) {
